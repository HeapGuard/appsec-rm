...
export default function App() {
  ...
  const route = () => { switch (page) { case 'dashboard': return <DashboardPage state={api.state} onTopic={openTopic} onPage={setPage} />; case 'roadmap': return <RoadmapPage state={api.state} onTopic={openTopic} onCheckpoint={api.setCheckpoint} />; case 'today': return <TodayPage state={api.state} onSave={api.saveLog} cases={api.state.practiceCases} />; case 'pomodoro': return <PomodoroPage cases={api.state.practiceCases} />; case 'practice': return <PracticePage state={api.state} onSave={api.savePracticeCase} onDelete={api.removePracticeCase} />; case 'resources': return <LibraryPage state={api.state} onTopic={openTopic} />; case 'activity': return <ActivityPage state={api.state} onDelete={api.removeLog} />; case 'reviews': return <ReviewsPage state={api.state} onAnswer={api.answerReview} onTopic={openTopic} />; case 'report': return <ReportPage state={api.state} />; case 'data': return <DataPage state={api.state} onExport={api.markExported} onImport={api.importState} onReset={api.reset} onSettings={api.setSettings} />; } };

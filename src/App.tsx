...
export default function App() {
  ...
  const route = () => { switch (page) { case 'dashboard': return <DashboardPage state={api.state} onTopic={openTopic} onPage={setPage} />; case 'roadmap': return <RoadmapPage state={api.state} onTopic={openTopic} onCheckpoint={api.setCheckpoint} />; case 'today': return <TodayPage state={api.state} onSave={api.saveLog} />; case 'resources': return <LibraryPage state={api.state} onTopic={openTopic} />; case 'activity': return <ActivityPage state={api.state} onDelete={api.removeLog} />; case 'reviews': return <ReviewsPage state={api.state} onAnswer={api.answerReview} onTopic={openTopic} />; case 'report': return <ReportPage state={api.state} />; case 'data': return <DataPage state={api.state} onExport={api.markExported} onImport={api.importState} onReset={api.reset} onSettings={api.setSettings} />; } };
  const day = daysBetween(api.state.settings.startDate) + 1;
  return <><Chrome page={page} onPage={setPage} compact={api.state.settings.compactMode}>{route()}</Chrome>{!api.state.settings.crtEnabled && <style>{'.app:before{display:none}'}</style>}<div className="day-counter">ДЕНЬ {day}</div>{topic && <TopicPanel topic={topic} progress={api.state.progress[topic.id]} onClose={() => setTopicId(undefined)} onSave={patch => api.setProgress(topic.id, patch)} />}{boot && <div className="boot" aria-live="polite"><div><b>INITIALIZING APPSEC TRAJECTORY</b><span>LOCAL STORAGE ........ OK</span><span>CURRICULUM ........... LOADED</span><span>OPERATOR ............. READY</span></div></div>}</>;
}
